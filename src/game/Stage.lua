...
function Stage:update(dt)
    ...
    if player.state == "attack" or player.state == "runAttack" then
        local hitbox = player:getHitbox()
        if hitbox then
            for k = 1, #self.mobs do
                local mob = self.mobs[k]
                if mob and not mob.died then
                    local mobHurt = mob:getHurtbox()
                    if mobHurt and hitbox:collision(mobHurt) and not mob.invincible then
                        mob:hit(10, player.dir)
                    end
                end
            end
        end
    ...
    for k = #self.mobs, 1, -1 do
        ...
        local mobHitbox = mob:getHitbox()
        local playerHurtbox = player:getHurtbox()
        if mobHitbox and playerHurtbox and mobHitbox:collision(playerHurtbox) then
            if player.state ~= "hurt" and player.state ~= "dead" and not mob.died then
                player:hit(mob.damage or 10)
                mob.state = "attack"
            end
        end
